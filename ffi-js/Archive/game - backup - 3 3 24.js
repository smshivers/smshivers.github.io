...
We want the game to increment on its own.
Whose turn is it?
The player's.
Which Hero?
Hero 1.
...
class Enemy extends Pawn {
    ...
    die() {
        console.log(this.name, "died!");
        gameUi.notification("terminated", "Terminated", 4000);
            ...
            setTimeout(function() {
                switch (heroes[i].command) {
                    case "fight" :
                        ...
                        gameUi.notification("target", heroes[i].target, 4000);
                        heroes[i].fight(heroes[i].target);
                        gameUi.notification("damage", heroes[i].attack, 4000);
                        break;
                    case "magic" :
